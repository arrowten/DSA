...
void preorder_traversal(std::unique_ptr<Tree_Node>& T, std::vector<int>& result) {
    if(!T) {
        return;
    }
    ...
    preorder_traversal(T->left, result);    // Traverse the left subtree
    preorder_traversal(T->right, result);   // Traverse the right subtree
}

TEST(Preorder_traversal, handles_various_cases) {
    ...
    std::vector<int> v;
    const std::vector<int> result = {1, 2, 4, 5, 3, 6, 7};
