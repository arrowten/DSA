/*
Given an integer array arr[] of N integers and an integer K. Your task is to return K largest elements in decreasing order.

Examples:

#Input 1: arr[] = [1, 23, 12, 9, 30, 2, 50], K= 3
#Output 1: 50 30 23

#Input 2: arr[] = [12, 5, 787, 1, 23], K= 2
#Output 2: 787 23

#Input 3: arr[] = [11, 5, 12, 9, 44, 17, 2], K= 2
#Output 3: 44 17
...
TEST(K_largest_elements, handles_various_cases) {
    ...
    std::vector<int> result = k_largest_elements(nums, 3), result2 = k_largest_elements(nums2, 2), result3 = k_largest_elements(nums3, 2);

    EXPECT_EQ(result, (std::vector<int>{50, 30, 23}));
    EXPECT_EQ(result2, (std::vector<int>{787, 23}));
    EXPECT_EQ(result3, (std::vector<int>{44, 17}));
}